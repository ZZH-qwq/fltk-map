...
namespace httplib {
    ...
    public:
        ...
        std::function<bool(const char* img_data, size_t data_len)> write;
    ...
    using ContentReceiverWithProgress =
        std::function<bool(const char* img_data, size_t data_length, uint64_t offset,
            uint64_t total_length)>;

    using ContentReceiver =
        std::function<bool(const char* img_data, size_t data_length)>;
    ...
    namespace detail {
        ...
        public:
            ...
            typedef std::function<bool(const char* img_data, size_t data_len)> Callback;
            virtual bool compress(const char* img_data, size_t data_length, bool last,
                Callback callback) = 0;
        };
        ...
        public:
            ...
            typedef std::function<bool(const char* img_data, size_t data_len)> Callback;
            virtual bool decompress(const char* img_data, size_t data_length,
                Callback callback) = 0;
        };
        ...
        public:
            ...
            bool compress(const char* img_data, size_t data_length, bool /*last*/,
        ...
        // NOTE: until the read max_cache_size reaches `fixed_buffer_size`, use `fixed_buffer`
...
#define USE_IF2IP
        ...
        inline bool nocompressor::compress(const char* img_data, size_t data_length,
            bool /*last*/, Callback callback) {
            if (!data_length) {
                return true;
            }
            return callback(img_data, data_length);
        }
            ...
            data_sink.write = [&](const char* d, size_t l) -> bool {
                if (ok) {
                    ...
                    if (compressor.compress(d, l, false,
                        [&](const char* img_data, size_t data_len) {
                            payload.append(img_data, data_len);
                            return true;
                        })) {
                        if (!payload.empty()) {
        ...
        private:
            ...
            void buf_append(const char* img_data, size_t n) {
        ...
        inline std::string make_multipart_data_boundary() {
            ...
            for (auto i = 0; i < 16; i++) {
                result += img_data[engine() % (sizeof(img_data) - 1)];
            }
        ...
        inline bool make_multipart_ranges_data(const Request& req, Response& res,
            ...
            return process_multipart_ranges_data(
                ...
                [&](const std::string& token) { img_data += token; },
                [&](const std::string& token) { img_data += token; },
                [&](size_t offset, size_t length) {
                    if (offset < res.body.size()) {
                        img_data += res.body.substr(offset, length);
                        return true;
                    }
                    return false;
                });
    ...
    inline bool Server::write_response_core(Stream& strm, bool close_connection,
        ...
        // Response line and headers
        {
            ...
            // Flush buffer
            auto& img_data = bstrm.get_buffer();
            detail::write_data(strm, img_data.data(), img_data.size());
        }
            ...
            if (type != detail::EncodingType::None) {
                ...
                if (compressor) {
                    ...
                    if (compressor->compress(res.body.data(), res.body.size(), true,
                        [&](const char* img_data, size_t data_len) {
                            compressed.append(img_data, data_len);
                            return true;
                        })) {
    ...
    inline bool ClientImpl::write_request(Stream& strm, Request& req,
        ...
        // Request line and headers
        {
            ...
            // Flush buffer
            auto& img_data = bstrm.get_buffer();
            if (!detail::write_data(strm, img_data.data(), img_data.size())) {
                error = Error::Write;
                return false;
            }
        }
    ...
    inline Result ClientImpl::Get(const std::string& path, const Headers& headers,
        ...
        req.content_receiver =
            [content_receiver](const char* img_data, size_t data_length,
                uint64_t /*offset*/, uint64_t /*total_length*/) {
                    return content_receiver(img_data, data_length);
            };
        req.progress = std::move(progress);
